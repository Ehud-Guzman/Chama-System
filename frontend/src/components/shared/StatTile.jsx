...
// `hint` is an optional line under the figure, for a total made of parts worth
// naming (the money carried forward inside an all-time total, say).
export default function StatTile({ label, value, hint, accent }) {
  return (
    <div className="rounded-xl border border-rule bg-surface p-4 md:p-5">
      ...
      {hint && <p className="amount mt-1 text-xs text-muted">{hint}</p>}
    </div>
  );
}
